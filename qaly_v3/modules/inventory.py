...
import pandas as pd
from modules.utils import load, save, save_dict, load_dict, get_theme_colors
    ...
    with tab3:
        ...
        if low:
            ...
            <div class="qcard" style="border:1px solid #f8717140;background:#3a101020;margin-bottom:1rem;">
                <div style="font-size:14px;font-weight:600;color:#f87171;margin-bottom:8px;">{len(low)} material{'s' if len(low)>1 else ''} need restocking</div>
            """, unsafe_allow_html=True)
            for i in low:
                st.markdown(f"""
                <div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid {C['BORDER']};font-size:13px;">
                    ...
                    <span style="color:#f87171;">Stock: {i['stock']} {i['unit']} &nbsp;(min {i['reorder']})</span>
                </div>
                """, unsafe_allow_html=True)
