...
from modules.utils import load, get_theme_colors, USER_COLORS, MEMBERS
...
def show():
    ...
    with col_l:
        ...
        if recent:
            for o in recent:
                ...
                <div class="qcard" style="padding:10px 14px;display:flex;align-items:center;justify-content:space-between;">
                    <div><div style="font-size:13px;font-weight:600;color:{C['TEXT']};">{o.get('name','?')}</div>
                    <div style="font-size:11px;color:{C['TEXT2']};margin-top:2px;">{o.get('product','?')} &nbsp;·&nbsp; {o.get('channel','?')} &nbsp;·&nbsp; {o.get('date','?')}</div></div>
                    <div style="text-align:right;"><div style="font-size:14px;font-weight:700;color:{C['ACCENT']};">RM {o.get('total',0):.2f}</div>
                    <span class="badge {bc}" style="margin-top:3px;display:inline-block;">{o.get('status','?')}</span></div>
                </div>
                """, unsafe_allow_html=True)
        else:
        ...
        # Production summary
        st.markdown(f"<div style='font-size:15px;font-weight:600;color:{C['TEXT']};margin-bottom:12px;'>Latest Production</div>", unsafe_allow_html=True)
        ...
        if batches:
            ...
            for b in latest:
                ...
                st.markdown(f"""
                <div class="qcard" style="padding:10px 14px;display:flex;align-items:center;justify-content:space-between;">
                    <div><div style="font-size:13px;font-weight:600;color:{C['TEXT']};">Batch {b.get('batch_no','?')} &nbsp;·&nbsp; {b.get('product','?')}</div>
                    <div style="font-size:11px;color:{C['TEXT2']};margin-top:2px;">{b.get('volume_L',0)}L &nbsp;·&nbsp; {b.get('units_produced',0)} units &nbsp;·&nbsp; {b.get('date','?')}</div></div>
    ...
    with col_r:
            ...
            for e in upcoming:
                ...
                <div class="qcard" style="padding:10px 14px;">
                    <div style="display:flex;justify-content:space-between;align-items:flex-start;">
                        <div><div style="font-size:13px;font-weight:600;color:{C['TEXT']};">{e.get('title','?')}</div>
                        <div style="font-size:11px;color:{C['TEXT2']};margin-top:2px;">{edate} &nbsp;·&nbsp; {e.get('location','')}</div></div>
        ...
        if active_batches:
            ...
            <div class="qcard" style="border:1px solid {C['SUCCESS_T']}40;background:{C['SUCCESS']};">
                <div style="font-size:13px;font-weight:600;color:{C['SUCCESS_T']};">{len(active_batches)} Active Production Batch{'es' if len(active_batches)>1 else ''}</div>
    ...
    with col_act:
        ...
        if recent_v:
            for v in recent_v:
                try:    ts = datetime.fromisoformat(v["timestamp"]).strftime("%d %b %Y &nbsp;·&nbsp; %I:%M %p")
                except: ts = v.get("timestamp","")
                ...
                <div style="display:flex;align-items:center;gap:10px;padding:7px 0;border-bottom:1px solid {C['BORDER']};">
                    ...
                    <div><div style="font-size:13px;font-weight:500;color:{C['TEXT']};">{v.get('user','?')}</div>
                    <div style="font-size:11px;color:{C['TEXT2']};">{ts}</div></div>
                </div>
                """, unsafe_allow_html=True)
        else:
